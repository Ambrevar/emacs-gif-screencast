...
scrolling in Eshell), no frame will be captured.  A workdaround consist in
...
- It won't render self-animated content such as auto-scrolling, also this is
  workable.

- It's should be much lighter on memory and CPU than video recording.
...
timestamps to set the appropriate delays to each frame.
